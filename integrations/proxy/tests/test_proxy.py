...
Shared infra (fake upstream, rig fixture, helpers) lives in conftest.py."""
...
from conftest import DONE_LINE, chunk, full_msg, iter_sse_lines, post, raw_post, wait_ends
...
def test_chain_fallback_non_streaming(rig):
    ...
    rig.state["verdicts"] = ["unfaithful", "faithful"]
...
def test_next_mode(rig):
    ...
    assert "unfaithful" in nudge["content"] and "Cats can fly." in nudge["content"]
    # delivered once only
